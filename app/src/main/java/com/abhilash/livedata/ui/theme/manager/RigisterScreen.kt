    ...
    myRef.setValue(employePen)
        .addOnCompleteListener { task ->
            if (task.isSuccessful) {
                Toast.makeText(context, "EmployeePen uploaded successfully", Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(context, "Failed to upload EmployeePen: ${task.exception?.message}", Toast.LENGTH_SHORT).show()
            }
        }
}
